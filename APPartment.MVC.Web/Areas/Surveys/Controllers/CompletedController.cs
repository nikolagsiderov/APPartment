...
namespace APPartment.MVC.Web.Areas.Surveys.Controllers
{
    ...
    public class CompletedController : BaseCRUDController<SurveyDisplayViewModel, SurveyPostViewModel>
    {
        ...
        {
            var model = await APPI.RequestAsync<TakeSurveyPostViewModel>(new string[] { CurrentAreaName, nameof(PendingController).Replace("Controller", ""), nameof(PendingController.TakeSurvey), id.ToString() }, CurrentUserID, CurrentHomeID);
            return View(model);
        }

        public override async Task SetGridItemActions(SurveyDisplayViewModel model)
        {
            model.ActionsHtml.Add(GridItemActionBuilder.BuildCustomAction(APPAreas.Surveys, nameof(ViewSurvey), model.ID, "btn-outline-warning", "fas fa-eye"));
        }
